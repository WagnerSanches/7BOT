    ...
    var n = document.URL.substring(p+1);
    var a = n.split("&");
    ...
    if(a[0] == "login") {
        ...
        console.log(a[1].indexOf("%20"))

        while(frase.indexOf("%20") > 0) {
            frase = frase.replace("%20", " ")
        }
        console.log(frase.indexOf("erro") + " 1")

        if(frase.indexOf("erro") == 0) {
            document.querySelector("#erro").textContent = frase.split("=")[1];
        } else {
            document.querySelector("#sucessoLogin").textContent = frase;;
        }

    }
    if(a[0] == "cadastro" && a[1].indexOf("erro") >= 0) {
        showAndHide(frm_log, frm_cad);
        var erro = document.querySelector("#erro-cad");
        var frase = a[1].split("=")[1];
